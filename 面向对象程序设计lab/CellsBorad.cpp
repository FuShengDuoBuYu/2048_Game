...
using namespace std;
vector<Cell> CellsBorad::createCells(int ROW,int COL){
    //创建格子初始值为0
...
int CellsBorad::updateBorad(char order,int ROW,int COL){
...
int CellsBorad::createRand2(vector<Cell> &cells){
...
int CellsBorad::upUpdateCells(int ROW, int COL){
    int addScore = 0;
    //对每一列依次向上进行合并和移动
    for (int i = 0; i < COL; i++)
    {
...
int CellsBorad::downUpdateCells(int ROW, int COL){
    int addScore = 0;
    //对每一列依次向下进行合并和移动
    for (int i = 0; i < COL;i++){
        //对第i列进行合并
...
int CellsBorad::leftUpdateCells(int ROW, int COL){
    int addScore = 0;
    //对每一行依次向左进行合并和移动
    for (int i = 0; i < ROW;i++){
        //对第i行进行合并
...
int CellsBorad::rightUpdateCells(int ROW, int COL){
    int addScore = 0;
    //对每一行依次向右进行合并和移动
    for (int i = 0; i < ROW;i++){
        //对第i行进行合并
...
bool CellsBorad::deadBoard(int endNum){
    for (int i = 0; i < cells.size();i++){
...
//判断移动方向的方法
int* CellsBorad::moveDirection(int ROW, int COL){
    int directionNum = 0;
